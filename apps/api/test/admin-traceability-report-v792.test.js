...
test('v7.9.7 incorpora pestaña administrativa Reportes con descarga y envío', () => {
...
test('API declara dependencia PDFKit y frontend v7.9.7', () => {
  assert.ok(pkg.dependencies?.pdfkit);
  assert.equal(pkg.version, '7.9.7');
  assert.match(config, /TP_APP_VERSION = "7\.9\.7"/);
});


test('v7.9.7 usa logos institucionales y flujo PDF anclado al margen izquierdo', () => {
...
test('v7.9.7 contabiliza altas de empresa por la cuenta COMPANY y no por edición del perfil', () => {
...
test('v7.9.7 nombre de archivo comienza con YYMMDD-HHmm para orden cronológico', () => {
