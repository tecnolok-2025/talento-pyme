...
test('v7.9.11 usa cola persistente y no SMTP sincrónico desde el botón', () => {
...
test('v7.9.11 fija límite conservador máximo de 450 comunicaciones por día', () => {
...
test('v7.9.11 continúa automáticamente al día siguiente', () => {
...
test('v7.9.11 procesa una sola campaña por vez y respeta orden FIFO', () => {
...
test('v7.9.11 separa envíos con pacing persistente', () => {
...
test('v7.9.11 reintenta errores temporales sin depender del administrador', () => {
...
test('v7.9.11 vuelve a comprobar la baja antes de cada envío', () => {
  assert.match(api, /resolveCommunicationRecipient/);
  assert.match(api, /bulkEmailOptOutAt/);
  assert.match(api, /SKIPPED_OPTOUT/);
});

test('v7.9.11 panel muestra progreso, límite diario y cantidad en cola', () => {
...
test('v7.9.11 incorpora cancelación de emergencia de pendientes', () => {
