...
test('v7.9.16 usa cola persistente y no SMTP sincrónico desde el botón', () => {
...
test('v7.9.16 fija límite conservador máximo de 450 comunicaciones por día', () => {
...
test('v7.9.16 continúa automáticamente al día siguiente', () => {
...
test('v7.9.16 procesa una sola campaña por vez y respeta orden FIFO', () => {
...
test('v7.9.16 separa envíos con pacing persistente', () => {
...
test('v7.9.16 reintenta errores temporales sin depender del administrador', () => {
...
test('v7.9.16 vuelve a comprobar la baja antes de cada envío', () => {
  assert.match(api, /resolveCommunicationRecipient/);
  assert.match(api, /bulkEmailOptOutAt/);
  assert.match(api, /SKIPPED_OPTOUT/);
});

test('v7.9.16 panel muestra progreso, límite diario y cantidad en cola', () => {
...
test('v7.9.16 incorpora cancelación de emergencia de pendientes', () => {
