...
test('v7.9.12 incorpora envio de respuesta de operador tambien por email', () => {
...
test('v7.9.12 permite reenviar ultimo mensaje informado por email', () => {
...
test('v7.9.12 utiliza correo institucional existente y no agrega credenciales', () => {
...
test('v7.9.12 muestra el contenido real del chat operador', () => {
  assert.doesNotMatch(admin, /__KEEP__/);
  assert.match(admin, /esc\(m\.content \|\| ''\)/);
});

test('v7.9.12 no requiere cambio de schema para los envios', () => {
