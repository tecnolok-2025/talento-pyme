    ...
    function renderAdminPlans(){
      ...
      document.getElementById('adminPlanRows').innerHTML = rows.map((plan)=> `<tr><td><input class="input" data-plan-field="name" data-code="${plan.code}" value="${esc(plan.name)}" /></td><td><input class="input" data-plan-field="days" data-code="${plan.code}" value="${plan.days}" /></td><td><input class="input" data-plan-field="price" data-code="${plan.code}" value="${plan.price}" /></td><td><input class="input" data-plan-field="publications" data-code="${plan.code}" value="${plan.publications || 0}" /></td><td><input class="input" data-plan-field="searches" data-code="${plan.code}" value="${plan.searches || 0}" /></td></tr>`).join('');
    }
    ...
    document.getElementById('btnOpenCheckout').onclick = ()=> { if(!state.cart.length){ document.getElementById('couponMsg').textContent = 'Primero agregá al menos un plan.'; return; } setTab('plans'); document.getElementById('checkoutCard').style.display=''; prefillCheckout(); renderCheckout(); document.getElementById('checkoutCard').scrollIntoView({ behavior:'smooth', block:'start' }); };
    document.getElementById('btnBackToCart').onclick = ()=> { document.getElementById('checkoutCard').style.display='none'; window.scrollTo({ top:0, behavior:'smooth' }); };
    document.getElementById('btnCheckoutConfirm').onclick = async ()=> {
      const msg = document.getElementById('checkoutMsg'); msg.textContent='Verificando compra…';
      try{
        const payload = { items: getQuoteItems(), couponCode: state.couponCode, billing: { razonSocial: document.getElementById('billingName').value.trim(), cuit: document.getElementById('billingTaxId').value.trim(), condicionFiscal: document.getElementById('billingTaxCondition').value.trim(), provincia: document.getElementById('billingProvince').value.trim(), localidad: document.getElementById('billingCity').value.trim(), calle: document.getElementById('billingStreet').value.trim(), numero: document.getElementById('billingNumber').value.trim(), piso: document.getElementById('billingFloor').value.trim(), depto: document.getElementById('billingDept').value.trim(), codigoPostal: document.getElementById('billingPostalCode').value.trim(), email: document.getElementById('billingEmail').value.trim() }, payment: { cardNumber: document.getElementById('cardNumber').value.trim(), cardHolder: document.getElementById('cardHolder').value.trim(), cardBrand: document.getElementById('cardBrand').value.trim(), expiry: document.getElementById('cardExpiry').value.trim(), cvv: document.getElementById('cardCvv').value.trim() } };
        const data = await apiFactory('/factory/checkout', { method:'POST', body: JSON.stringify(payload) });
        msg.textContent = data.message || 'Compra verificada.'; state.cart=[]; saveCart(); state.couponCode=''; state.couponFeedback=''; localStorage.removeItem('tp_factory_coupon'); document.getElementById('checkoutCard').style.display='none'; state.selectedOrderId = data.order?.id || null; await loadBootstrap(); setTab('summary');
      }catch(e){ msg.textContent = e.message; }
    };
